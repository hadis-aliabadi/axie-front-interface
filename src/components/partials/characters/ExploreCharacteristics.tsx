...
export const ExploreCharacteristics = ({jsonFile}:{jsonFile:string}) => {
  const { t } = useTranslation()
  return (
    <section id="explore" className="max-w-full mx-16 px-4 sm:px-6 mt-28">
      <IntroductionBox vertical className=" ">
        <div className="flex flex-col items-center gap-16 lg:py-8 lg:px-4">
          ...
          <div className="flex flex-wrap justify-center items-stretch gap-10 ">
            {t<string, axies_types[]>(`${jsonFile}:axies_types`, {
              returnObjects: true,
            }).map((item, index) => (
              <Link href={item.link}>
                <BoldableWrapper
                  key={index}
                  className="flex flex-col justify-between  gap-4 relative h-[300px]"
                >
                  <Image
                    src={item.src}
                    width={200}
