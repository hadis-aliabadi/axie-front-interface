...
const Header = () => {
  ...
  return (
    <header className={`w-full sm:max-w-[600px] md:max-w-[900px] xl:max-w-full mx-auto px-4 sm:px-6 ${poppins.className}`}>
      <div className="flex justify-between items-center py-2">
        ...
        >
          {active ? (
            ...
            <i className="fa-solid fa-bars text-greypale text-2xl absolute z-50">
            </i>
          )}
        </div>
        {/* //*SideNav Starts */}
        {!!active && (
          <nav className="w-screen min-h-screen overflow-y-auto inset-0 absolute z-20 top-0 left-0  px-16 bg-white animate-slide lg:hidden ">
                ...
                return (
                  ...
                  >
                    {item.subMenu ? (
                      ...
                      <Link
                        className="text-grey font-semibold p-2 mx-2 rounded inline-flex items-center hover:text-pink "
                        href={item.link}
                      >
                        {item.title}
                      </Link>
                    ...
                    {item?.subMenu && (
                      ...
                      >
                        {item.subMenu.map((item) => (
                          <Link
                            href={item.link}
                            className="h-14 flex items-center py-2 px-2 ms-7  hover:text-pink hover:outline-none font-semibold"
                            key={item.link}
                          >
                            {item.title}
                          </Link>
