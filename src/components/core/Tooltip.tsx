...
export const Tooltip = ({ render, tooltipText,className }: Props) => {
  ...
  return (
    ...
    >
      {render({})}
      {showTooltip && (
        <div className={`w-40 absolute top-full -left-1/3 translate-y-3 bg-black text-white p-1 rounded-lg text-[11px] font-medium ${className}`}>
          {tooltipText}
        </div>
      )}
    </div>
